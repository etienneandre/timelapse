...
#
    ...
    method = BestAlignment.HOMOGRAPHY
    h = h_affine
    deviation = -1

    if err_affine > ERROR_THRESHOLD and err_homog > ERROR_THRESHOLD:
        use_affine = True
        method = BestAlignment.FALLBACK_AFFINE
        print(f"⚠️ Valeur aberrante détectée : err_affine = {err_affine} ; err_homog = {err_homog}")
    else:
        if err_affine < err_homog:
            use_affine = True
            method = BestAlignment.AFFINE
        else:
            use_affine = False
            method = BestAlignment.HOMOGRAPHY
...
def align_folder(input_dir, output_dir):
    ...
    for fname in tqdm(filenames[1:], desc="Alignement des images"):
        ...
        total_deviation += deviation if method != BestAlignment.ERROR else 0
        nb_non_errors += 1 if method != BestAlignment.ERROR else 0
        # TODO: mieux de d'abord créer l'image puis ajouter le texte, et enfin l'exporter dans un fichier
    ...
    for alignment in BestAlignment:
